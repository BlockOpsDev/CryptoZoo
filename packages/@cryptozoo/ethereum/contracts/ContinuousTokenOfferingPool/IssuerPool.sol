...
import "./SwapIssuer/SwapIssuer.sol";

/**
 * @title Balancer Issuer Pool
 * @notice A Balancer Pool implementation for issuing tokens on swap
 *
 * @dev Issue and Redeem tokens based Constant Reserve Ratio Math
 */
abstract contract IssuerPool is IMinimalSwapInfoPool, BasePool, SwapIssuer {
  // Swap Hook Handler
  function onSwap(
    ...
    SwapReceipt memory swapReceipt = _calcSwap(
      swapType,
  ...
  /**
   * @dev Called after any swap.
   * Account for tokens that are issued and redeemed through a pool swap
   */
